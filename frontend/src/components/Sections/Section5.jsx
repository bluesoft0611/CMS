...
const Section5 = () => {
  ...
  return (
    <div className="bg-blue-50 py-12 h-[1202px] flex flex-col justify-center">
      <div className="container mx-auto px-4 lg:px-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-[1150px] mx-auto">
          {page?.cards?.map((card, index) => (
            <div
              key={index}
              className="p-6 bg-white shadow-sm gap-4 items-start max-w-[560px] min-h-[344px]"
            >
              <img src={`${baseURL}${card?.icon?.url}`} alt="Icon" className="w-10 h-10 mb-6" />
              <div>
                <p class="font-nunito font-medium text-[24px] leading-[32px] tracking-normal mb-4">
                  {card.title}
                </p>
                <p class="font-dm font-normal text-[18px] leading-[30px] tracking-normal opacity-80">
                  {card.description}
                </p>
              </div>
            </div>
